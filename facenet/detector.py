...
class Network(object):
    ...
    def load(self, data_path, sess, ignore_missing=False):
...
def create_mtcnn(sess):
    with tf.variable_scope('pnet'):
        ...
        pnet = PNet({'data': data})
        pnet.load(os.path.join('facenet', 'det1.npy'), sess)
    with tf.variable_scope('rnet'):
        data = tf.placeholder(tf.float32, (None, 24, 24, 3), 'input')
        rnet = RNet({'data': data})
        rnet.load(os.path.join('facenet', 'det2.npy'), sess)
    with tf.variable_scope('onet'):
        data = tf.placeholder(tf.float32, (None, 48, 48, 3), 'input')
        onet = ONet({'data': data})
        onet.load(os.path.join('facenet', 'det3.npy'), sess)

    pnet_func = lambda img: sess.run(
        ('pnet/conv4-2/BiasAdd:0', 'pnet/prob1:0'),
        {'pnet/input:0': img}
    )
    rnet_func = lambda img: sess.run(
        ('rnet/conv5-2/conv5-2:0', 'rnet/prob1:0'),
        {'rnet/input:0': img}
    )
    onet_func = lambda img: sess.run(
